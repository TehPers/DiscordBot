...
namespace BotV2.Services.Data.Resources.Objects
{
    public class RedisObjectResource<T> : RedisResource, IObjectResource<T>
    {
        ...
        public virtual async Task<Option<T>> Get()
        {
            ...
            var result = await db.StringGetAsync(this.ResourceKey);
        ...
        {
            var db = await this.DbFactory.GetDatabase();
            var result = await db.StringGetSetAsync(this.ResourceKey, this.Serializer.ToString(value));
            return result.HasValue ? new Option<T>(this.Serializer.FromString<T>(result)) : default;
        }
        ...
        {
            var db = await this.DbFactory.GetDatabase();
            return await db.StringSetAsync(this.ResourceKey, this.Serializer.ToString(value), when: When.NotExists);
        }
